...
const Cruise = () => {
  ...
  const getdata = async ()=>{
    ...
    const res = await fetch("https://seeandamans.onrender.com/api/user/get/packages")
 ...
 
    {
      datas?.allpackages?.map((i)=>{
        return(
          ...
          <Maindiv  className="card" style={{cursor:'pointer'}}>
         
          <Img src={i.image2?.url} />
          <Innerdiv>
           <P1>Power Pack deals on Cabs</P1>
           <P2>Book cab and more @ upto {i.offers} off</P2>
           <Book   onClick={click} style={{cursor:'pointer'}}>Book Now</Book>
          </Innerdiv>
